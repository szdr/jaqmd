...
**核心となる差別化点:**
- bigram / 形態素解析 / ベクトルの3つの日本語向け検索方式を提供
...
- 形態素解析・ベクトル検索は**オプション依存**とし、最小構成では bigram 検索のみで動作させる
...
- query expansion は独自開発する（別リポジトリ `jaqmd-qe` を想定）

### 段階的なインデックス構築を尊重する

- `update`（bigram）は常時、`morph`（形態素）と `embed`（ベクトル）はオプション
...
-- FTS1: bigram（trigram tokenizer、辞書不要・デフォルト）
...
bigram FTS は documents の更新に追従してトリガーで自動同期する。
...
- クエリ変換時は trigram の前方一致を活用する（実装で詳細を詰める）
...
- bigram と形態素で検索結果が期待通り変わることを確認する
